...
```ruby
bitbucket.repos.list 'vongrippen', 'bitbucket' do |repo|
    puts repo.slug
end
```
...
The `bitbucket_api` gem will use the default middleware stack which is exposed by calling `stack` on client instance. However, this stack can be freely modified with methods such as `insert`, `insert_after`, `delete` and `swap`. For instance to add your `CustomMiddleware` do
...
Original bitbucket_api gem Copyright (c) 2011-2012 Piotr Murach. See LICENSE.txt for further details.
